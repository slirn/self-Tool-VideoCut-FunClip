...
- **M2 UI 层**：清单按父段分组渲染子段表 + 标记徽章 + 翻转 + 保存端点；播放器复用 `slirn-cut-player`——段级预播（playCutAt + 播到段尾自动停）与父段「试听保留效果」（keep 段连续跳播）；`resplit_segment` 端点 + 组头「✂️ 重新切分」编辑框（预填 target_text、回写修订 user_note）；主按钮状态化（已生成→「🔄 重新执行切分修剪」，清 manual_marks 重算落盘、阶段状态不变）+ 修订更新黄条（saved_at 晚于 cutlist created_at 提示过期）。E2E 只读验证渲染与预播。
