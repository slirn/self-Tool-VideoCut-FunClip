...
def test_render_revision_zone_states(tmp_path: Path, monkeypatch):
    ...
    # 状态跳转（REQ-20260917-024/025）：状态下拉（独立于筛选 chips）+ 上一条/下一条按钮
    assert 'id="slirn-rev-jump-sel"' in h3 and 'slirn-rev-jump-sel' in h3
    ...
    assert "点击行定位播放" not in h3 and "点击行连续跳播" in h3, "连续跳播提示（REQ-20260917-028）"
    assert "展开模型分析与修剪/更正内容" in h3
